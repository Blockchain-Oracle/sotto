...
async function verifiedPrepare(intent: HumanPurchaseLedgerIntent) {
  ...
  return await verifyHumanPreparedPurchaseHash(observation, {
...
it("atomically checkpoints one authentically verified prepared purchase", async () => {
  ...
  try {
    ...
    const verified = await verifiedPrepare(claimed!.intent);
