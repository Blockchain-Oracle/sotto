...
if (!new Set(["copy", "verify"]).has(mode)) {
  throw new Error("Usage: context-sync.mjs <copy|verify> --source <archive-root>");
}
...
function inside(root, path) {
  ...
  return value !== ".." && !value.startsWith(`..${sep}`) && !resolve(path).startsWith(`${root}${sep}..`);
}

async function digest(path) {
  const stat = await lstat(path);
  if (stat.isSymbolicLink() || !stat.isFile()) throw new Error(`Unsafe file: ${path}`);
...
for (const entry of manifest.entries) {
  ...
  if (!entry.destination.startsWith(".thoughts/") || !inside(sourceRoot, source) || !inside(destinationRoot, destination)) {
    throw new Error(`Path rejected: ${entry.destination}`);
  }
  const sourceDigest = await digest(source);
  if (sourceDigest.bytes !== entry.bytes || sourceDigest.sha256 !== entry.sha256) {
  ...
  if (destinationDigest.bytes !== entry.bytes || destinationDigest.sha256 !== entry.sha256) {
    throw new Error(`Destination checksum mismatch: ${entry.destination}`);
  }
  expected.add(entry.destination.slice(".thoughts/".length));
}

const actual = new Set(await listFiles(resolve(destinationRoot, ".thoughts"), resolve(destinationRoot, ".thoughts")));
const extra = [...actual].filter((path) => !expected.has(path));
const missing = [...expected].filter((path) => !actual.has(path));
if (missing.length || (strict && extra.length)) {
  throw new Error(`Context set differs: ${extra.length} extra, ${missing.length} missing`);
}
process.stdout.write(`${mode} verified ${manifest.entries.length} exact context files${extra.length ? `; ${extra.length} local files outside the imported manifest` : ""}\n`);
