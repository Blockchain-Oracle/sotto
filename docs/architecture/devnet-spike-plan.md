...
Earn the product architecture with real Five North evidence. This plan does not
build the production marketplace or translate the prototype into runtime code.

## Phase 1 - Clean Split

- Preserve payroll at `c29e4da` with an archive tag.
- Create fresh local and Git histories.
- Track a self-contained product router and public authority documents.
- Restore private context only through an exact checksum manifest.
- Verify repository IDs and SHAs before any GitHub handover.
...
## Phase 4 - Minimal Daml Privacy Package

- Deploy an x402-specific research DAR, independent of payroll.
...
- Demonstrate that the unrestricted baseline signer bypasses a separate policy.
- Test a real constrained signer/funding candidate from available Canton
  capabilities, not an invented API.
- Bind policy, signer, transfer, and evidence to one canonical live-402 purchase
  commitment.
- Attack challenge swaps, price drift, recipient/asset mutation, relay
  re-preparation, proof replay, expiry, and policy revision changes.
- Require one combined path: fresh 402, atomic policy/transfer/context, accepted
  settlement proof, retry of the same request, authentic 200, private context,
  and outsider absence.
- Test settled-payment/provider-delivery failure without automatic repayment.
...
Unit mocks and isolated test fixtures are allowed. No simulated payment,
fabricated update ID, fixture transaction, LocalNet-only run, payroll DAR, or
prototype transition can satisfy DevNet or product evidence.
