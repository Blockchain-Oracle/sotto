...
function requireHumanActive(scope: PrepareOnlyScope): void {
...
export async function prepareOnlyHumanPurchase(
  ...
  try {
    requireHumanActive(scope);
    const walletPreflight = await callback(scope, "wallet preflight", () =>
      input.createWalletPreflight(scope.signal),
    );
    requireHumanActive(scope);
    const paymentObservation = await createHumanPaymentObserver(
      input.fetchAuthorized,
    )(input.request, { signal: scope.signal });
    requireHumanActive(scope);
    const packageSelection = await callback(scope, "package selection", () =>
      input.claimPackageSelection(
        Object.freeze({
          ...
          executeBefore: executeBefore(paymentObservation.observedAt),
